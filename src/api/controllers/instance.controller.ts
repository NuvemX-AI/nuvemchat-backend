...
export class InstanceController {
    ...
    try {
      ...
      try {
        this.chatwootService.create(instance, {
          ...
          nameInbox:
            instanceData.chatwootNameInbox ?? instance.instanceName.split('-cwId-')[0],
          number: instanceData.number,
  ...
  public async fetchInstances({ instanceName, instanceId, number }: InstanceDto, key: string) {
    ...
    if (env.KEY !== key) {
      // PATCH: corrigido acesso .prisma.instance.findMany
      const instancesByKey = await this.prismaRepository.prisma.instance.findMany({
        where: {
          token: key,
