...
const { v4: uuidv4 } = require('uuid'); // substitui crypto.randomUUID
...
router.post('/webhook/instagram', (req, res) => {
  ...
  // TODO: processar mensagens/comentários aqui
  return res.sendStatus(200);
});

//--------------------------------------------------------------------
// 2) Geração de URL para Facebook Login
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
// 3) Callback após autenticação do Instagram
//--------------------------------------------------------------------
router.get('/instagram/callback', async (req, res) => {
  try {
    ...
    // 2) Buscar páginas com conta IG vinculada
    ...
    if (!page) {
      return res.status(400).json({
        ...
        hint: 'No app do Instagram: Configurações → Conta profissional → Centro de Contas → vincule a Página.'
      });
    }
    ...
    console.log('✅ IG vinculado: @' + username);

    // 4) Persistir no banco (se houver req.db)
    if (req.db) {
      ...
      console.warn('⚠️ req.db ausente – integração não foi salva no banco.');
    }

    // 5) Redirecionar para frontend
    const redirectUrl = `${process.env.FRONTEND_URL || 'http://localhost:8080'}/integrations/instagram/success`;
